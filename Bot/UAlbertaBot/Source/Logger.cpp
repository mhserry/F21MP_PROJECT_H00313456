...
void Logger::LogAppendToFile(const std::string & logFile, const std::string & msg)
{
    ...
    {
        //BWAPI::Broodwar->printf("Warning: Logging File: %s doesn't exist..", logFile);
    }
    else 
    {
...
{
    std::fstream logStream(logFile.c_str());
    logStream << msg;
    logStream.flush();
    logStream.close();
}
...
bool UAlbertaBot::FileUtils::FileExists(const std::string& filename)
{

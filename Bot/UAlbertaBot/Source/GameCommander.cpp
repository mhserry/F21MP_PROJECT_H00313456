...
void GameCommander::update()
{
	...
	//Strategy manager
	m_timerManager.startTimer(TimerManager::Strategy);
	Global::Strategy().onUpdate();
	m_timerManager.stopTimer(TimerManager::Strategy);
	...
	if((BWAPI::Broodwar->getFrameCount() % m_LateUpdateFrames) == 0) 
	{
		lateUpdate();
	}
...
void UAlbertaBot::GameCommander::lateUpdate()
{
	Global::Strategy().onLateUpdate();
}
